...
export default async function DocumentosCategoriasPage() {
  const lang = await getLang()
  const labels = t(lang)

  const categorias = await fetchDocumentosCategorias(lang)
        ...
        <div className="px-20 pb-24 space-y-20 mt-16">
          <div className="grid grid-cols-4 gap-16">
            {categoriasTransformadas.map((categoria: any) => (
              <Link
                key={categoria.id}
                href={`/documentos/categoria/${categoria.id}`}
              ...
              >
                ...
                <div className="p-10 space-y-6 mb-12">
          ...
          {categoriasTransformadas.length === 0 && (
            <div className="text-center py-32">
              <p className="text-4xl text-muted-foreground font-medium">{labels.documentsNoCategories}</p>
            </div>
          )}
        </div>
      </div>
