...
// Fetch Materiais_e_trabalhos_Categorias
...
// Fetch Materiais_e_trabalhos by category
export async function fetchMateriaisByCategoria(categoriaId: string | number, lang: string = 'pt', tipo: string = 'Material') {
  const data = await directus.request(
    readItems('Materiais_e_trabalhos', {
      fields: ['*', 'translations.*', 'capa', 'categoria', 'edificio'],
      filter: { 
...
// Fetch all Materiais_e_trabalhos
export async function fetchMateriais(lang: string = 'pt', tipo: string = 'Material', edificioId?: string | number) {
  const filter: any = { tipo: { _eq: tipo } }
  ...
  const data = await directus.request(
    readItems('Materiais_e_trabalhos', {
      fields: ['*', 'translations.*', 'capa', 'categoria.id', 'categoria.translations.*', 'edificio'],
      filter,
      sort: ['ordem'],
      deep: {
        translations: {
