import { cookies } from "next/headers"

export type Lang = "pt" | "en"

export async function getLang(): Promise<Lang> {
  try {
    const c = (await cookies()).get("lang")?.value
    return c === "en" ? "en" : "pt"
  } catch {
    return "pt"
  }
}

const labels = {
  pt: {
    buildings: "Edifícios",
    buildingsDesc: "As instalações da escola ao longo da sua história",
    viewDetails: "Ver detalhes",
    yearBuilt: "Ano de construção",
    location: "Localização",
    aboutBuilding: "Sobre este edifício",
    publications: "Publicações",
    publicationsDesc: "Publicações da escola ao longo dos anos",
    type: "Tipo",
    back: "Voltar",
    sort: "Ordenar",
    all: "Todos",
    sortByYearNewest: "Ano (Mais Recente)",
    sortByYearOldest: "Ano (Mais Antigo)",
    sortByNameAZ: "Nome (A-Z)",
    sortByNameZA: "Nome (Z-A)",
    // Menu
    people: "Pessoal",
    students: "Alunos",
    courses: "Cursos",
    coursesDesc: "Disciplinas lecionadas ao longo da história da escola",
    materials: "Materiais",
    works: "Trabalhos",
    services: "Serviços",
    servicesDesc: "Os serviços oferecidos pela escola",
    startDate: "Data de início",
    endDate: "Data de fim",
    active: "Ativo",
    inactive: "Inativo",
    activePeriod: "Período ativo",
    aboutService: "Sobre este serviço",
    gallery: "Galeria",
    status: "Estado",
    groups: "Grupos",
    groupsDesc: "Os grupos da escola",
    aboutGroup: "Sobre este grupo",
    galleryDesc: "Galeria de fotos da escola",
    category: "Categoria",
    subcategory: "Subcategoria",
    photos: "Fotos",
    subcategories: "Sub-Categorias",
    photo: "Foto",
    openMap: "Abrir no OpenStreetMap",
    // Documentos
    documents: "Documentos",
    documentsDesc: "Categorias de documentos",
    documentsNoCategories: "Nenhuma categoria encontrada.",
    documentsNoDocumentsInCategory: "Nenhum documento encontrado nesta categoria.",
    documentsLoadCategoryError: "Ocorreu um erro ao carregar a categoria de documentos.",
    authors: "Autores",
    nameUnavailable: "Nome não disponível",
    descriptionUnavailable: "Descrição não disponível",
    contentUnavailable: "Conteúdo não disponível",
    authorUnavailable: "Autor não disponível",
    categoryUnavailable: "Categoria não disponível",
    typeUnavailable: "Tipo não disponível",
    noCategoriesFound: "Nenhuma categoria encontrada.",
    // Cursos
    yearRange: "Intervalo de anos",
    resetFilters: "Repor filtros",
    lectiveYears: "Anos letivos",
    sortByLectiveYearsMost: "Anos letivos (Mais)",
    sortByLectiveYearsLeast: "Anos letivos (Menos)",
    allBuildings: "Todos os edifícios",
    notAvailable: "N/D",
    // Pessoal
    peopleDesc: "Lista de pessoal da escola",
    processNumber: "Nº Processo",
    name: "Nome",
    teacher: "Docente",
    yes: "Sim",
    no: "Não",
    search: "Pesquisar",
    searchPlaceholder: "Pesquisar por nome ou nº processo",
    filterPeople: "Filtrar",
    allPeople: "Todos",
    onlyTeachers: "Docentes",
    onlyNonTeachers: "Não Docentes",
    sortByProcessOldest: "Nº Processo (Mais antigo)",
    sortByProcessNewest: "Nº Processo (Mais recente)",
    results: "Resultados",
    page: "Página",
    of: "de",
    details: "Detalhes",
    lectiveYearsWorked: "Anos letivos trabalhados",
    taughtDisciplinesCount: "Disciplinas lecionadas",
  },
  en: {
    buildings: "Buildings",
    buildingsDesc: "The school's facilities throughout its history",
    viewDetails: "View details",
    yearBuilt: "Year built",
    location: "Location",
    aboutBuilding: "About this building",
    publications: "Publications",
    publicationsDesc: "School publications over the years",
    type: "Type",
    back: "Back",
    sort: "Sort",
    all: "All",
    sortByYearNewest: "Year (Newest)",
    sortByYearOldest: "Year (Oldest)",
    sortByNameAZ: "Name (A-Z)",
    sortByNameZA: "Name (Z-A)",
    // Menu
    people: "People",
    students: "Students",
    courses: "Courses",
    coursesDesc: "Subjects taught throughout the school's history",
    materials: "Materials",
    works: "Works",
    services: "Services",
    servicesDesc: "Services offered by the school",
    startDate: "Start date",
    endDate: "End date",
    active: "Active",
    inactive: "Inactive",
    activePeriod: "Active period",
    aboutService: "About this service",
    gallery: "Gallery",
    status: "Status",
    groups: "Groups",
    groupsDesc: "The school groups",
    aboutGroup: "About this group",
    galleryDesc: "School photo gallery",
    category: "Category",
    subcategory: "Subcategory",
    photos: "Photos",
    subcategories: "Subcategories",
    photo: "Photo",
    openMap: "OpenStreetMap",
    // Documents
    documents: "Documents",
    documentsDesc: "Document categories",
    documentsNoCategories: "No categories found.",
    documentsNoDocumentsInCategory: "No documents found for this category.",
    documentsLoadCategoryError: "An error occurred while loading the document category.",
    authors: "Authors",
    nameUnavailable: "Name not available",
    descriptionUnavailable: "Description not available",
    contentUnavailable: "Content not available",
    authorUnavailable: "Author not available",
    categoryUnavailable: "Category not available",
    typeUnavailable: "Type not available",
    noCategoriesFound: "No categories found.",
    // Courses
    yearRange: "Year range",
    resetFilters: "Reset filters",
    lectiveYears: "Lective years",
    sortByLectiveYearsMost: "Lective years (Most)",
    sortByLectiveYearsLeast: "Lective years (Least)",
    allBuildings: "All buildings",
    notAvailable: "N/A",
    // People
    peopleDesc: "School staff list",
    processNumber: "Process No.",
    name: "Name",
    teacher: "Teacher",
    yes: "Yes",
    no: "No",
    search: "Search",
    searchPlaceholder: "Search by name or process no.",
    filterPeople: "Filter",
    allPeople: "All",
    onlyTeachers: "Teachers",
    onlyNonTeachers: "Non-teachers",
    sortByProcessOldest: "Process No. (Oldest)",
    sortByProcessNewest: "Process No. (Newest)",
    results: "Results",
    page: "Page",
    of: "of",
    details: "Details",
    lectiveYearsWorked: "Lective years worked",
    taughtDisciplinesCount: "Disciplines taught",
  },
} as const

export function t(lang: Lang) {
  return labels[lang]
}


